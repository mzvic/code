...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.f_f_input.setText("4000")       
        arg_input_layout.addWidget(f_f_label)
        arg_input_layout.addWidget(self.f_f_input)  
        
        avg_time_label = QLabel("          Averaging period in spectrometer:")
        self.avg_time_input = QLineEdit(self)
        self.avg_time_input.setFixedWidth(60) 
        self.avg_time_input.setText("5")       
    ...
    def update_graph3(self):
        ...
        #self.data_matrix[1:] = self.data_matrix[:-1]  
        #self.data_matrix[0] = 0 
        self.data_matrix_avg[0, np.array(self.freq2, dtype=int)] = self.data2
        #AT = np.flipud(np.transpose(np.fliplr(self.data_matrix)))
