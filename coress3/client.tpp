...
void ClientUpstream<Outbound, Response>::EnqueueOutboundMessage(Outbound &outbound, const Timestamp &timestamp) {
  ...
  if (state_ != RUNNING) {
//	cout << "T:" << this_thread::get_id() << " PublisherClient: Trying to publish on a not running broker_client, no message has been published" << endl;
...
void ClientDownstream<Inbound, Request>::Start() {
  // Create new reactor
  client_downstream_reactor_ = new ClientDownstreamReactor<Inbound>([this](Inbound &inbound) { Enqueue(inbound); }, [this](bool ok) { OnClientDownstreamReactorOnDone(ok); });
...
//  cout << "T:" << this_thread::get_id() << " SubscriberClient: OnClientDownstreamReactorOnDone: " << ok << endl;
  LOG("Upstream client OnClientDownstreamReactorOnDone: " << ok);
...
template<class Inbound, class Request>
void ClientDownstream<Inbound, Request>::Enqueue(Inbound &inbound) {
